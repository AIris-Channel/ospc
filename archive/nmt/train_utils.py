...
class LossCompute:
    """A function to calculate loss, perform parameter backward propagation, and update training"""
...
def evaluate(data, model, mode='dev', use_beam=True):
    """Perform predictions using a trained model on data and print the translated results of the model"""
...
def translate(src, model, use_beam=True):
    """Use a trained model to predict a single sentence and print the translated result of the model"""
    _, tgt_decode = target_tokenizer_load(config.tgt_vocab_path)
    with torch.no_grad():
        model.load_state_dict(torch.load(lastest_checkpoint()))
        model.eval()
